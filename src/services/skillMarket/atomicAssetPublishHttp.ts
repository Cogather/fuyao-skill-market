...
export async function queryAtomicPublishOrganizations(
  ...
  const organizations = await queryHttpPublishableOrganizations(
    requiredText(userId, '尚未获取当前用户工号'),
    {
      dimType,
      dimCode: requiredText(asset.dimCode, '资产缺少维度编码，无法查询目标组织'),
    },
  );
  ...
  const response = await skillBaseService.publishHarnessAssets(
    {
      ...
      userName: requiredText(input.userName, '尚未获取当前用户姓名'),
    },
    {
      items: [
        {
      ...
      orgCode: requiredText(input.organizationCode, '请选择目标组织'),
    },
  );
...
export async function retryAtomicAssetPublish(taskId: string, userId: string): Promise<void> {
  const response = await skillBaseService.retryHarnessAssetPublish(
    requiredText(taskId, '发布记录缺少任务 ID'),
