...
export function isOpsDashboardBundle(value: unknown): value is OpsDashboardBundle {
 ...
 */
export function parseOpsDashboardFetchResult(
  res: Response,
  bodyText: string,
): ApiEnvelope<OpsDashboardBundle> {
  let parsed: unknown;
  try {
  ...
  return {
    ...
    message:
      '运营看板接口：未返回 OpsDashboardBundle（需含 kpi、deptTree、orgBars、topSkills，或与 Mock 下 JSON 文件同结构）',
    data: emptyOpsDashboardBundle(),
  };
}
