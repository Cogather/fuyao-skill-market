...
import { isSkillMarketApiTransport, normalizeSkillMarketTransport } from './transport';

export type { SkillMarketTransport } from './transport';

/**
 * 根据环境变量选择 Mock（内存）或 HTTP（真实后端）实现。
 *
 * **正式接口（`VITE_SKILL_MARKET_TRANSPORT=https` 或 `http`）** 时只会走 `createSkillMarketHttpClient`，
...
export function createSkillMarketClient(
  ...
  const mode = normalizeSkillMarketTransport();
  const baseUrl = import.meta.env.VITE_SKILL_MARKET_API_BASE ?? '';
  if (isSkillMarketApiTransport(mode)) {
