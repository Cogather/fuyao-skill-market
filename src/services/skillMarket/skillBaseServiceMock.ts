...
import { isSkillMarketApiTransport } from './transport';
...
function withPageMeta<T extends object>(items: T[], total: number): T[] {
  const out = items.map((item) => ({ ...item })) as T[] & { records?: T[]; total?: number };
  out.records = out;
  out.total = total;
  return out;
}
...
function makeSkillMd(record: Pick<MockSkillRecord, 'name' | 'description' | 'requirements' | 'author' | 'currentVersion' | 'category' | 'tags'>): string {
  return [
    '---',
    `name: ${record.name}`,
...
function pageSkills(params: Record<string, unknown>, source = skillRecords): MockSkillRecord[] {
...
function upsertUploadedSkill(file: File | undefined): MockSkillRecord {
  ...
  const newSkill = toMockSkillRecord({
    id: String(Math.max(0, ...skillRecords.map((s) => Number(s.id)).filter(Number.isFinite)) + 1),
    skill_id: name,
    name,
...
function createSkillFromBody(body: Record<string, unknown>): MockSkillRecord {
  ...
  const record = toMockSkillRecord({
    ...
    downloads: 0,
    rating: 4.5,
    version: readString(body.version, '1.0.0'),
    versions: [
      {
        version: readString(body.version, '1.0.0'),
        publishTime: stamp,
        note: 'Mock 手动创建',
        packageFileName: `${name}-v${readString(body.version, '1.0.0')}.zip`,
        packageSize: 0,
      },
    ],
    ownedByUser: true,
...
function shouldUseMock(): boolean {
  ...
  return !isSkillMarketApiTransport();
}
