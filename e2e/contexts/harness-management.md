...
- Agent / Skill 资产面板：`#harness-panel-assets`，HTTP 和 Mock 均只显示 Agent / Skill / Command / Extension 四个类型，默认选中 Agent，无「全部」入口；包含详情、Skill 评估报告和发布流程。工具栏操作按钮固定按「＋ 新增 / 导入 / 导出」排列，统一使用白底描边样式；点击新增直接使用当前类型页签打开 Agent / Skill / Command 新增弹窗，不再二次选择类型，Extension 页签下禁用新增。
