...
- Skill 的「内容 / 评估报告」保持原有紧凑版本选择器；进入「行为评测」后由页签内的独立版本选择器展示「已评测 / 部分评测 / 进行中 / 失败 / 未评测」，Skill 详情不再展示大块灰色版本上传信息区。行为评测由统一 service 按 `VITE_SKILL_MARKET_TRANSPORT` 分流：HTTP 模式调用 `GET/POST /api/v1/harness/plans/skill/behavior-eval` 和 `GET /api/v1/harness/plans/skill/behavior-eval/trend`，Mock 模式只访问内存模拟数据，不发上述请求。`TRIGGER` / `QUALITY` 各自查询、各自空态和报告模板；进行中状态每 30 秒轮询且可手动刷新，终态停止；仅进行中禁止重复发起，失败终态可重试。趋势保持两条可能稀疏的独立序列。HTTP 回归为 `harness-assets-behavior-eval-http.spec.ts`，Mock 边界回归为 `harness-assets-mock.spec.ts`。
