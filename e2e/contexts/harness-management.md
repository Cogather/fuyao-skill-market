...
- Agent / Skill 资产面板：`#harness-panel-assets`，HTTP 和 Mock 均只显示 Agent / Skill / Command / Extension 四个类型，默认选中 Agent，无「全部」入口；包含详情、Skill 评估报告、行为评测和发布流程。Skill 详情页签固定按「内容 / 评估报告 / 行为评测 / 发布记录」排列，行为评测稳定锚点为 `#asset-detail-tab-behavior` 与 `#asset-detail-panel-behavior`，内层 `role=tablist` 的 aria-label 为「评测类型」。工具栏操作按钮固定按「＋ 新增 / 导入 / 导出」排列，统一使用白底描边样式；点击新增直接使用当前类型页签打开 Agent / Skill / Command 新增弹窗，不再二次选择类型，Extension 页签下禁用新增。
- Skill 的「内容 / 评估报告」保持原有紧凑版本选择器；进入「行为评测」后由页签内的独立版本选择器展示「已评测 / 部分评测 / 进行中 / 失败 / 未评测」，Skill 详情不再展示大块灰色版本上传信息区。Agent / Skill / Command / Extension 详情的版本信息在 HTTP 模式读取 `GET /api/v1/harness/plans/components/detail` 中当前版本项的 `reportUrl`，字段缺失时兼容 `repoUrl`；Mock 模式从版本仓地址透传或生成稳定的模拟来源仓地址；有合法 HTTP(S) 地址时展示为「来源仓」链接并在新页面打开，切换版本时同步更新。行为评测由统一 service 按 `VITE_SKILL_MARKET_TRANSPORT` 分流：HTTP 模式调用 `GET/POST /api/v1/harness/skill-eval/behavior-eval` 和 `GET /api/v1/harness/skill-eval/behavior-eval/trend`，Mock 模式只访问内存模拟数据，不发上述请求。`TRIGGER` / `QUALITY` 各自查询、各自空态和报告模板；进行中状态不自动轮询，仅通过「刷新状态」手动查询；仅进行中禁止重复发起，失败终态可重试。趋势保持两条可能稀疏的独立序列。HTTP 回归为 `harness-assets-behavior-eval-http.spec.ts`，Mock 边界回归为 `harness-assets-mock.spec.ts`。
...
- 2026-09-22：Skill 资产详情在「评估报告」后新增「行为评测」，并把「发布记录」统一放到最后。行为评测已接入查询记录、发起评测和版本趋势三组接口；HTTP / Mock 由统一 service 分流，覆盖两种 mode 的独立空态、进行中轮询、失败重试、动态概要/分布图、用例筛选与展开和稀疏趋势。Agent / Command / Extension 不显示该页签。回归：`harness-assets-behavior-eval-http.spec.ts`、`harness-assets-mock.spec.ts`。
