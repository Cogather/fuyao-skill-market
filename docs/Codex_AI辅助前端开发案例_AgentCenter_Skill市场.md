...
本案例的核心价值不在于“AI 写了多少行代码”，而在于形成了一套可复制的人机协作方式：**人负责定义目标、提供上下文、建立约束并完成最终判断；AI 负责理解复杂信息、快速展开方案、执行跨文件修改并辅助验证。** 双方不是简单的“人提需求、AI 写代码”，而是在连续反馈中共同把业务目标转化为可交付结果。

| 协作要点                 | 人的关键投入                                                         | AI 的主要作用                                                | 最终形成的结果                                              |
| ------------------------ | -------------------------------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------- |
| 用结构化约束代替模糊指令 | 明确业务目标、用户角色、验收标准，并提供类型定义、接口契约和禁止事项 | 在确定的边界内补全方案、识别遗漏并生成一致的实现             | AI 获得充分发挥空间，但不会偏离业务方向                     |
| 让 AI 进入真实项目上下文 | 指定相关需求、现有页面和不能破坏的行为，要求 AI 先理解仓库再修改     | 分析目录、依赖和调用链，在现有架构中完成跨文件连续迭代       | 产出能够融入当前系统，而不是无法落地的孤立代码片段          |
| 用双数据通道支持并行推进 | 人确定前后端契约、业务规则和联调标准，并判断哪些能力可以先行验证     | 同步维护 Mock 与 HTTP 适配，使页面在后端未完全就绪时仍可运行 | 前端、后端和产品验证可以并行，提前形成可演示的业务闭环      |
| 用验证闭环完成质量收敛   | 人负责业务验收、视觉判断、权限确认和联调反馈，决定结果是否真正可用   | 执行 lint、build、问题定位和修改复核，并根据反馈持续修正     | 将“AI 生成代码”逐步收敛为经过检查、验收和联调的“可交付代码” |
